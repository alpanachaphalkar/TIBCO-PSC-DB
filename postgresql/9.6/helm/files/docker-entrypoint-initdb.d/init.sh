...
psql -U "${PGUSER}" -d "${PGDATABASE}" -v psc_mdmdb_user="${psc_mdmdb_user}" -v psc_mdmdb_password="'${psc_mdmdb_password}'" -v psc_super_user="${psc_super_user}" -v psc_mdmdb_schema="${psc_mdmdb_schema}" -v psc_mdmdb_database="${psc_mdmdb_database}" -h "${PG_HOST}" -p "${PG_PORT}" -f "${SCRIPTPATH}/postgres/V202105101000__psc_initial.sql"
